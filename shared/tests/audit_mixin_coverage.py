...
def assert_full_audit_mixin_coverage(
    ...
    """Imports every module under models_package so a model missing from __init__.py can't skip this check."""
    ...
    missing = sorted(
        ...
        if not issubclass(mapper.class_, AuditColumnsMixin)
        and mapper.class_.__name__ not in exempt
    )
